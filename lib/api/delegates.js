...
module.exports = function (app) {
	...
	function Active() {
		const getForgedTimeout = 10 * 11;
		this.getActive = function (cb) {
			request.get({
				url: `${app.get('lisk address')}/delegates/?sort=rank:asc&limit=11`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	function Standby(n) {
		...
		this.actualOffset = (isNaN(this.offset)) ? 11 : this.offset + 11;
	...
	this.getNextForgers = function (temp, error, success) {
		request.get({
			url: `${app.get('lisk address')}/delegates/forgers?limit=11&offset=0`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: (err || 'Response was unsuccessful') });
